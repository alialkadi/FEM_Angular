...
export class CategoryTypeService {
  ...
  getAllCategoriestypes(
    ...
    page?: number,
    pageSize?: number,
  ): Observable<ApiResponse<CategoryTypeListResponse>> {
    let params = new HttpParams().set('all', all);

    if (page !== undefined) params = params.set('page', page);
    if (pageSize !== undefined) params = params.set('pageSize', pageSize);
    return this._http.get<ApiResponse<CategoryTypeListResponse>>(
  ...
  CreateCategoryType(
    formData: FormData,
  ): Observable<ApiResponse<CategoryType>> {
  ...
  getTypesByCategory(
    ...
    return this._http.get<ApiResponse<CategoryTypeListResponse>>(
      this.baseurl + `/CategoryTypes/byCategory/${id}`,
    );
  }
  ...
  moveDown(id: number): Observable<ApiResponse<boolean>> {
    return this._http.patch<ApiResponse<boolean>>(
      `${this.baseurl}/CategoryTypes/${id}/move-down`,
      {},
    );
  }
}
